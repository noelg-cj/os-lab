#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <unistd.h>

#define N 5
#define THINKING 2
#define HUNGRY 1
#define EATING 0
#define LEFT (phnum+4)%N
#define RIGHT (phnum+1)%N

int state[N];
int philosopher[N] = { 0, 1, 2, 3, 4 };

sem_t mutex;
sem_t S[N];

int check_availability(int phnum) {
	if (state[phnum] == HUNGRY && state[LEFT] != EATING && state[RIGHT] != EATING) {
		state[phnum] = EATING;
		sleep(2);

		printf("Philosopher %d acquires chopstick %d and chopstick %d.\n", phnum+1, LEFT+1, phnum+1);
		printf("Philosopher %d is eating.\n", phnum+1);

		sem_post(&S[phnum]);
	}
}

void take_chopstick(int phnum) {
	sem_wait(&mutex);
	state[phnum] = HUNGRY;

	printf("Philosopher %d is hungry.\n", phnum+1);

	check_availability(phnum);
	sem_post(&mutex);

	sem_wait(&S[phnum]);
	sleep(1);
}

void drop_chopstick(int phnum) {
	sem_wait(&mutex);
	state[phnum] = THINKING;

	printf("Philosopher %d is putting chopstick %d and %d down.\n", phnum+1, LEFT+1, phnum+1);
	printf("Philosopher %d is thinking.\n", phnum+1);

	check_availability(LEFT);
	check_availability(RIGHT);

	sem_post(&mutex);
}

void *init(void *num) {
	while(1) {
		int *i = num;
		sleep(1);
		take_chopstick(*i);
		sleep(0);
		drop_chopstick(*i);
	}
}

int main(void) {
	int i;
	pthread_t thread_id[N];

	sem_init(&mutex, 0, 1);
	for (i = 0; i < N; i++) {
		sem_init(&S[i], 0, 0);
	}

	for (i = 0; i < N; i++) {
		pthread_create(&thread_id[i], NULL, init, &philosopher[i]);
 
        printf("Philosopher %d is thinking\n", i + 1);
	}

	for (i = 0; i < N; i++)
 		pthread_join(thread_id[i], NULL);
}
